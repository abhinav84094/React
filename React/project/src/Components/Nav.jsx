...
function Nav() {
  ...
  return (
    <nav className='w-[20%] h-full bg-zinc-200 overflow-hidden' >
            ...
            <div className='text-xl pt-10 text-center'><Link to="/addnewitem" className='border border-black rounded-md p-2 m-1'>Add New Product</Link></div>
                <hr className='h-0.5 mt-5 bg-blue-200' />
                <h1 className='text-xl m-2'>category</h1>
                <div>
                    {distinct_Category.map((val, index)=>{
                        return (<Link to={`/?category=${val}`} key={index} className='flex items-center'>
